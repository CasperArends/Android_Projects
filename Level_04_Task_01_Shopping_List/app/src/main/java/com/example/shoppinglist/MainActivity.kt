...
class MainActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        fabAddActivity.setOnClickListener { addProduct() }
    }
    ...
    private fun validateFields(): Boolean {
        return if (etShoppingItem.text.toString().isNotBlank() && etItemAmount.text.toString().isNotBlank()) {
            true
        } else {
    ...
    private fun addProduct() {
        if (validateFields()) {
            mainScope.launch {
                val product = Product(
                    name = etShoppingItem.text.toString(),
                    quantity = etItemAmount.text.toString().toInt()
                )
